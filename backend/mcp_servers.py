...
if massive_api_key:
    market_params = {
        "command": "uvx",
        # mcp_massive's own package metadata doesn't pin the mcp SDK version, so uvx
        # otherwise resolves the latest mcp (2.x), which renamed FastMCP -> MCPServer
        # and breaks mcp_massive's code (written for mcp 1.x). Pin it here the same
        # way the fetch server below pins it.
